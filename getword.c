 ...
 *   - -2 if the word is an ESCAPED ampersand (\&)
...
int getword(char* w) {
    ...
    // bool_ampersandEscaped is used later to check if an ampersand (&) has been escaped, if so, return -2
    int bool_ampersandEscaped = 0;
    
    int bool_escapedTilde = 0;
    ...
    while((nextchar = getchar())) {
        ...
        if(bool_isescaped != 0 && ( nextchar != EOF)) {
            ...
            if(nextchar == ';' ||  nextchar == '\n' || nextchar == EOF) {
                ...
                // This section handles the case where the $ is by itself.
                nextchar = getchar();
                
                // If the $ is not alone, go here:
                if(!(nextchar == ' ' || nextchar == '\n') && index == 0) {
                    ungetc(nextchar, stdin);
                    bool_isnegative = 1;
                    continue;
    ...
    if(bool_ampersandEscaped) {
        return -256;
    }
    
    if(bool_escapedTilde) {
        return -258 - index;
    }
